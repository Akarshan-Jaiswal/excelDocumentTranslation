...
location =("E:\\notes\\Python\\document translation tool\\testxl.xlsx")
...
for index in range (totalsheetcount):
    ...
    sheetinfo.append([index,testsheet.ncols,testsheet.nrows])
print(sheetinfo)
sheetdata=[]
sheettrans=[]
for index in sheetinfo:
    tempsheet=testworkbook.sheet_by_index(index[0])
    for colindex in range (index[1]):
        for rowindex in range (index[2]):
            tempdata=tempsheet.cell_value(rowindex,colindex)
            if tempdata =="":
                continue
            sheetdata.append([index[0],colindex,rowindex,tempdata])
            temptrans=translator.translate(str(tempdata),dest='ja',src='en')
...
writeworkbook.save('E:\\notes\\Python\\document translation tool\\transtext.xls')
